...
try:
    ...
    for conf in tqdm(configurations[:N]):
        ...
        with open(template) as template_file:
            ...
            if model_name == 'sar_multi_victim.xml':
                tplt_content = tplt_content.replace("**N_V**", str(N_V))
                tplt_content = tplt_content.replace("**PI_S**",
                                                    '{' + ','.join([str(x) for x in model_params['pi_survivors']['s1']])
                                                    + '}')
        ...
        out_path = config['UPPAAL SETTINGS']['UPPAAL_OUT_PATH'].format(os.getcwd(),
                                                                       ...
                                                                       '_' + str(FIXED_PARAMS['timebound']) +
                                                                       '_' + str(conf[model_params["plot_dim_1"]]) +
                                                                       '_' + str(conf[model_params["plot_dim_2"]]))
        os.system("{} {} {} {}".format(script_path, uppaal_path, new_model, out_path))

        with open(out_path) as out_file:
            lines = out_file.readlines()
            EXP_VALUE_PRE = " -- Formula: E[<=TAU;10000](max: r_payoff)\n"
            EXP_VALUE_POST = " -- Formula: E[<=TAU;10000](max: r_payoff) under {}\n".format(model_params['strat_name'])
    ...
    axs[0].set_title('Exp. Value of Task Duration\np-value: {:.1e},\n'
